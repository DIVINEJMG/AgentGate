...
export default function AgentsPanel({ organization, user, apiVersion, onApiVersionChange, onCountChange }: Props) {
    ...
    return <>
        ...
        {registerOpen && <div className="modal-backdrop"><section className="modal-card" role="dialog" aria-modal="true" aria-label="Register agent"><div className="modal-title"><div><p className="panel-kicker">AGENT IDENTITY</p><h2>Register agent identity</h2></div><button className="icon-button" aria-label="Close registration" onClick={() => setRegisterOpen(false)}><X size={16} /></button></div><form className="agent-form" onSubmit={handleRegister}><label htmlFor="agent-name">Agent name</label><input id="agent-name" value={name} onChange={(event) => setName(event.target.value)} maxLength={80} placeholder="Support Agent" autoFocus /><label htmlFor="agent-description">Purpose</label><textarea id="agent-description" value={description} onChange={(event) => setDescription(event.target.value)} maxLength={240} placeholder="Handles customer support triage and drafts responses." /><div className="credential-note"><KeyRound size={17} /><div><strong>A credential will be issued once.</strong><p>Audoryn stores only its SHA-256 hash. The plaintext credential cannot be recovered later.</p></div></div><button className="primary-button" disabled={submitting}>{submitting ? 'Registering…' : 'Register agent'}</button></form></section></div>}

        {credentialReveal && <div className="modal-backdrop"><section className="modal-card credential-modal" role="dialog" aria-modal="true" aria-label="Agent credential"><div className="modal-title"><div><p className="panel-kicker">COPY ONCE</p><h2>Store this agent credential</h2></div></div><p className="modal-copy">This secret is shown only for this issuance. After you close this window, Audoryn will keep the fingerprint and hash — not the secret.</p><div className="credential-secret"><code>{credentialReveal.credential.secret}</code><button className="icon-button" aria-label="Copy credential" onClick={copyCredential}>{copied ? <Check size={16} /> : <Clipboard size={16} />}</button></div><div className="credential-meta"><span>Fingerprint {credentialReveal.credential.fingerprint}</span><span>Version {credentialReveal.credential.version}</span></div><button className="primary-button" onClick={() => { setCredentialReveal(null); setCopied(false); }}>{copied ? 'Credential stored' : 'I have stored this credential'}</button></section></div>}
