...
export async function disableManagedAgentForWorkerDeletion(organizationId:string,agentId:string,actorUserId:string){const current=await loadAgent(organizationId,agentId);if(current.status==='disabled'&&current.credentialStatus==='revoked')return publicAgent(agentId,current);const updatedAt=new Date().toISOString();const next:AgentRecord={...current,status:'disabled',credentialStatus:'revoked',updatedAt};const agent=await updateAgent(organizationId,agentId,next);await markCredentialRevoked(organizationId,current.credentialRecordId);await appendAuditEventBestEffort({organizationId,eventType:'agent.lifecycle.worker_deleted',category:'identity',severity:'warning',actor:{type:'human',id:actorUserId,label:null},resource:{type:'agent',id:agentId,name:agent.name},correlationId:null,outcome:'disabled',summary:'Automatically managed Agent Identity disabled because its Worker was deleted.',metadata:{previousStatus:current.status,credentialFingerprint:current.credentialFingerprint}});return agent}
