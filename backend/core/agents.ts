...
export async function getAgentForUser(userId: string, organizationId: string, agentId: string) { await requireAgentAccess(userId, organizationId, 'agents.read'); const record = await loadAgent(organizationId, agentId); if (record.organizationId !== organizationId) throw new AgentDomainError('Agent identity not found.', 404); return publicAgent(agentId, record); }
