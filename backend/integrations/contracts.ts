...
export interface ProviderCapabilityDescriptor{providerOperation:string;resourceType:string;action:string;target:string;scope:string;description:string;risk:CapabilityRisk;requiresApproval?:boolean;requiresCredential?:boolean}
...
export interface IntegrationAdapter{provider:IntegrationProvider;displayName:string;credentialPolicy:'optional'|'required'|'disabled';capabilities:ProviderCapabilityDescriptor[];supportedOperations:string[];validateConnection(config:Record<string,unknown>,credential?:string):Promise<ProviderConnectionResult>;checkHealth(config:Record<string,string>,credential?:string):Promise<ProviderHealthResult>;normalizeOperationInput?(providerOperation:string,input:Record<string,unknown>):Record<string,unknown>;executeOperation(config:Record<string,string>,providerOperation:string,credential?:string,input?:Record<string,unknown>):Promise<ProviderExecutionResult>}
