from sqlalchemy import DateTime, ForeignKey, MetaData, String, Table, Column, UniqueConstraint
...
migration_staging_records = Table(
    ...
    migration_metadata,
    Column("id", UUID(as_uuid=True), primary_key=True),
    Column("batch_id", UUID(as_uuid=True), ForeignKey("migration_batches.id", ondelete="CASCADE"), nullable=False),
    ...
    Column("created_at", DateTime(timezone=True), nullable=False),
    UniqueConstraint("batch_id", "entity_type", "legacy_id", name="uq_migration_staging_legacy"),
)
...
migration_id_mappings = Table(
    ...
    UniqueConstraint("source_system", "entity_type", "legacy_id", name="uq_migration_source_identity"),
)
