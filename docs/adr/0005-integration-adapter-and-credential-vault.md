...
## Context
Audoryn must support many external systems without coupling the canonical domain to GitHub, Slack, Gmail, n8n, MCP, or future providers. Provider credentials are more sensitive than ordinary integration metadata and cannot be persisted as plaintext.
...
GitHub is the first adapter. Repository coordinates are parsed as `owner/repository`; Audoryn hardcodes GitHub's API endpoint and does not fetch arbitrary user-supplied URLs.
